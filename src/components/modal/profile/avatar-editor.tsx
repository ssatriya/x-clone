...
type AvatarEditorProps = {
  avatarUrl: string;
  cropAvatar: { x: number; y: number };
  zoomAvatar: number;
  setZoomAvatar: (zoom: number) => void;
  setCroppedAvatarPixels: (value: any) => void;
  setCropAvatar: (value: any) => void;
};
...
export default function AvatarEditor({
  ...
  setZoomAvatar,
  setCropAvatar,
  setCroppedAvatarPixels,
}: AvatarEditorProps) {
  const onZoomAvatarChange = (zoom: number) => {
    setZoomAvatar(zoom);
  };
  ...
  const onCropAvatarComplete = (
    ...
    setCroppedAvatarPixels(croppedAreaPixels);
  };

  const onCropAvatarChange = (crop: any) => {
    setCropAvatar(crop);
  };
    ...
    <div className="h-full flex-col justify-between relative">
      <div className="">
        <Cropper
          ...
          crop={cropAvatar}
          zoom={zoomAvatar}
          onZoomChange={onZoomAvatarChange}
          onCropComplete={onCropAvatarComplete}
          onCropChange={onCropAvatarChange}
          aspect={1}
          showGrid={false}
          cropShape="rect"
        />
      ...
      <div className="absolute bottom-0 left-[50%] w-[50%] translate-x-[-50%] h-14 flex items-center justify-center">
        <div className="flex gap-3 items-center justify-center h-full">
          ...
          <Slider
            ...
            step={0.01}
            value={zoomAvatar}
            onChange={(zoom: number) => onZoomAvatarChange(zoom)}
            className="w-[300px]"
            aria-label="zoom"
          >
